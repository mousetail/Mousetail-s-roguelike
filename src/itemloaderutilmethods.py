...
def BasicitemFuncMaker(itemCls=items.Item, *extrastatslist):
    ...
    for i in extrastatslist:
        if i == RE_NORMAL_ARGS:
            extrastats.extend((("name", TYPE_VAR_NAME), ("pname", TYPE_STRING), ("weight", TYPE_INT),
                               ("fake name", TYPE_STRING), ("fake pname", TYPE_STRING),
                               ("range", TYPE_INT)))
        elif i == RE_STATIC_ARGS:
            extrastats.extend((("position", TYPE_VAR_POSITION), ("image_name", TYPE_IMAGE), ("cage", TYPE_VAR_CAGE),
                               ("world", TYPE_VAR_WORLD)))
        else:
    ...
    def internal1(name, stats):
        def internal2(position, world, cage, safemode=False, returnbody=False):
            # type: (list, world.World, imagecage.Cage, bool, bool) -> Item
            if returnbody:
                ...
                for i in extrastats:
                    ...
                    if i[1] == TYPE_STRING:
                        ...
                        for rect in stats[i[0]]:
                            ...
                            damage = (tuple(float(damagetag.find(i).text) for i in
                                            ("phis", "ele", "las", "hea", "ice", "rad", "bio", "gas")),)
                        ...
                        itm = cage.getProxy(stats[i[0]], True)
