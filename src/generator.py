...
class Generator(Grid):
    ...
    floortypes: Key: id, Value: indexes of types of floor, use constants
    ...
    def __init__(self, size, dlevel=0, objectdefinitions=None, atributes=None):
        if objectdefinitions is None:
            self.objectdefinitions = {"empty": TS_EMPTY,
                                      "roomtypes": ((7, (1,), (1,), (12,), ((TS_DOOR_LEFT, TS_DOOR_OPEN_LEFT),
                                                                            (TS_DOOR_RIGHT, TS_DOOR_OPEN_RIGHT,)),
                                                     False),
                                                    (1, (2,), (3,), (),
                                                     ((TS_LOCKED_DOOR_LEFT,), (TS_LOCKED_DOOR_RIGHT,)), True)
                                                    ),
                                      # frequency,(walltypes,),(floortypes,),(specialobjects,),(doors,),
                                      # spawnsobjects
                                      "walltypes": {
                                          1: ((TS_WALL_BOTTOM,), (TS_WALL_RIGHT,), (TS_WALL_LEFT,), (TS_WALL_TOP,)),
                                          2: ((TS_GREEN_WALL_1, TS_GREEN_WALL_2),)},
                                      "floortypes": {1: (TS_FLOOR_NORMAL, TS_FLOOR_SPECIAL),
                                                     3: (TS_GREEN_FLOOR_1, TS_GREEN_FLOOR_2,
                                                         TS_GREEN_FLOOR_3)},
                                      "specialobjectclasses": {1: 14},
                                      "paths": (11, 12, 13),
                                      "1perdungeon": ((TS_PAPRI, "TILE"), (TS_STAIRS_UP, "TILE", "DOWN"),
                                                      (TS_STAIRS_DOWN, "TILE", "UP")) + ((ITM_MONSTER, "OBJ"),) * 12 +
                                                     ((ITM_ITEM, "OBJ"),) * 48
    ...
    def finish(self):
                        ...
                        if self[x, y] == 0 or ((((self[x, y + 1] in tuple(itertoolschain(*room.doortype)) and (
                                        ...
                                        self[x + 1, y + 1] in self.objectdefinitions["paths"])) or
                                                     (self[x, y - 1] in tuple(itertoolschain(*room.doortype)) and (
                                                                     self[x - 1, y - 1] in self.objectdefinitions[
                                                                     "paths"] or
                                                                     self[x + 1, y - 1] in self.objectdefinitions[
                                                                     "paths"])) or
                            ...
                            if len(room.walltype) == 1:
                                ...
                                inx = 0
                                if (room.bounds[0] == x) or (
                                                x == room.bounds[0] + room.bounds[2] and room.bounds[1] + room.bounds[
                                            3] != y):
                                    inx |= 1
                                if (room.bounds[1] == y and room.bounds[0] != x) or (
                                            y == room.bounds[1] + room.bounds[3]):
                                    inx |= 2
        ...
        for room in self.rooms:  # Should change so no longer needed
            # The generator should check for errors itself
            for door in room.doors:
                ...
                for o in getadjacent(door, self.size):
                    if self[o] in self.objectdefinitions["paths"]:
                        remove = False
                if remove:
                    self[door] = random.choice(room.walltype[0])
    ...
    def findpath(self, pointa, pointb, extrarooms=(), cleanup=True):
        amountbacktracked = 0
        ...
        while walkposition != pointb:
            ...
            for i in (relpts + [(-direction[1], 0), (0, -direction[0]),
                ...
                nwalk = (walkposition[0] + i[0], walkposition[1] + i[1])
                ok = True
                for r in itertoolschain(self.rooms, extrarooms):
                    if r.iscorner(nwalk):
                        ok = False
                        reasons.pop(0)
